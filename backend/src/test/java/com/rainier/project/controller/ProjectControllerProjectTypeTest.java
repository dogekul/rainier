...
class ProjectControllerProjectTypeTest {
  ...
  /** TC-PROJTYPE-013/TC-CODE-01 (v0.0.49): 自动编号前缀随类型 — EXTERNAL_DELIVERY → ED-{id}. */
  @Test
  void post_autoCode_externalDeliveryPrefix() throws Exception {
    Long userId = createUser();
    ObjectNode body = json.createObjectNode();
    body.put("name", "X");
    body.put("ownerUserId", userId);
    ...
    mockMvc
        .perform(
            post("/api/projects").contentType(MediaType.APPLICATION_JSON).content(body.toString()))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.code", matchesPattern("ED-\\d+")));
  }

  /** TC-PROJTYPE-014/TC-CODE-03: CORE_TECH → CT-{id}. */
  @Test
  void post_autoCode_coreTechPrefix() throws Exception {
    Long userId = createUser();
    ObjectNode body = json.createObjectNode();
    body.put("name", "X");
    body.put("ownerUserId", userId);
    body.put("projectType", "CORE_TECH");
    mockMvc
        .perform(
            post("/api/projects").contentType(MediaType.APPLICATION_JSON).content(body.toString()))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.code", matchesPattern("CT-\\d+")));
  }

  /** TC-PROJTYPE-015/TC-CODE-04: 传入 code 被忽略，仍自动生成（CORE_FEATURE → CF-{id}）. */
  @Test
  void post_providedCodeIgnored_autoGenerated() throws Exception {
    Long userId = createUser();
    ObjectNode body = json.createObjectNode();
    body.put("code", "MANUAL-XYZ");
    body.put("name", "X");
    body.put("ownerUserId", userId);
    body.put("projectType", "CORE_FEATURE");
    mockMvc
        .perform(
            post("/api/projects").contentType(MediaType.APPLICATION_JSON).content(body.toString()))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.code", matchesPattern("CF-\\d+")));
  }
}
