# Capability: me-inbox — v0.0.42 (NEW)

> NEW capability。`GET /api/me/inbox` 自助 PO 需求收件箱 read-model：待处理诉求（未转化）+ 我的需求。all-users
> （token 必需，非 admin）。纯读聚合 Demand/DemandRequirementLink/Requirement，零写、零新表。路线图 §4 PO。
