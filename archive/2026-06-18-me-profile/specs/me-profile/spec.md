# Capability: me-profile — v0.0.40 (NEW)

> NEW capability。`GET /api/me/profile` 自助个人贡献/能力档案 read-model（org 身份 + 岗位 + 直接上级 + 贡献计数）。
> all-users（token 必需，非 admin）。纯聚合既有数据，零写、零新表。
