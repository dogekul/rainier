...
> NEW capability (v0.0.41-admin-compliance, 2026-06-18)。管理员合规仪表盘 read-model：审计活动聚合 +
> 停用-残留权限对账。admin-gated（AdminPaths Tier A `/api/compliance`）。纯读聚合 AuditLog/User/UserRole/Role，
> 零写、零新表。路线图 #10。见 [[frontend-scaffold]] / [[backend-authz]] / [[entity-audit-log]]。
...
用户 SHALL NOT 返回。注：残留授权是 inert（停用用户永不提升、不能登录），属 de-provisioning 卫生缺口（建议回收），
非活跃越权。
