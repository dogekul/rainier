# Capability: ai-work-log

> NEW capability (v0.0.43-ai-work-log, 2026-06-18) — **飞轮层底座**。AI 工作日志：AI 每个动作 = 一条带 evidence 的
> PROPOSED 提议，人类可见/可采纳/可驳回（PROPOSED→ACCEPTED/REJECTED 状态机；驳回数据=KPI 金矿）。all-users
> （token 必需）。新表 rainier_ai_work_log。**种子驱动壳——0 真实 AI/推断/外部集成**。路线图 §3/§4。见 [[frontend-scaffold]]。
