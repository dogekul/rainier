...
> 合并自 change `2026-06-23-project-types`（Phase 6）。项目类型由 轻量/正式 拓展为 轻量 + 三正式子类。见 [[opportunity]] / [[frontend-scaffold]]。
...
create 未指定类型 SHALL 默认 `CASUAL`；create/update 指定非法类型 SHALL 返回 400。`FORMAL` 退役（不在 ALL）。`project_type` 列 SHALL 宽 32（容纳 EXTERNAL_DELIVERY），既有 MySQL 列由启动 `ProjectTypeColumnWiden` 加宽。
...
- **WHEN** `POST /api/projects` body `projectType="FOO"`（或退役的 `FORMAL`）
- **THEN** SHALL 返回 400
...
> 合并自 change `2026-06-24-project-code-autogen`（Phase 6）。项目编号由「类型前缀 + 自增 id」自动生成。见 [[opportunity]] / [[frontend-scaffold]]。
